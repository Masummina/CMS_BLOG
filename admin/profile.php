...
?>
  ...
  ?>
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        if(isset($_SESSION['user_id']))
                        {
                        	...
                        	$db_select = mysqli_query($dbconnect, $profile_query);
                        	
                        	if($db_select){
                        		while ($row = mysqli_fetch_assoc($db_select)) 
                        		{
     ...
     if(isset($_POST['user_submit'])){
		...
		if(empty($user_image)){
			$user_edit_img = "SELECT * FROM users WHERE user_id = '{$user_id}'";
		...
		if(empty($username) || $username == ""){
			...
			$user_edit_query ="UPDATE `users` SET `username` = '{$username}', `user_password` = '{$password}', `user_firstname` = '{$fname}', `user_lastname` = '{$lname}', `user_email` = '{$user_email}', `user_image` = '{$user_image}', `user_role` = '{$user_role}' WHERE `users`.`user_id` = '{$user_id}'";
